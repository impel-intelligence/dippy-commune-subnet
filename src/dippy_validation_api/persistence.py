...
LEADERBOARD_TABLE = "leaderboard_commune"
...
class SupabaseState:
    ...
    def update_leaderboard_status(self, hash, status, notes=""):
        try:
            response = (
                self.client.table(LEADERBOARD_TABLE)
                .upsert(
    ...
    def get_json_result(self, hash):
        try:
            response = self.client.table(LEADERBOARD_TABLE).select("*").eq("hash", hash).execute()
    ...
    def remove_record(self, hash: str):
        try:
            status = self.client.table(LEADERBOARD_TABLE).delete(returning="minimal").eq("hash", hash).execute()
            return status
        except Exception as e:
    ...
    def update_row(self, row):
        ...
        try:
            response = self.client.table(LEADERBOARD_TABLE).upsert(row).execute()
            return response
        except Exception as e:
